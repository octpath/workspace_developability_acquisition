...
def test_code_format_and_counts():
    ...
    assert len(t) == 74 and len(h) == 53
    assert sorted(t, key=lambda x: int(x.split("-")[1][1:])) == [f"EXP-T{i:03d}" for i in range(1, 75)]
