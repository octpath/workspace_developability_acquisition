...
```text
residue → residue interaction → antibody-level representation
```
...
```text
xᵢ = xᵢ(content) + xᵢ(pos) + xᵢ(chain) + xᵢ(IMGT) + xᵢ(region)
```
...
```text
xᵢ(content) = Embedding(AAᵢ)
```

PLM の場合:

```text
xᵢ(content) = W_proj · hᵢ(PLM)
```
...
```text
r⁽ˡ⁺¹⁾ = Attention(Q = r⁽ˡ⁾, K = X⁽ˡ⁾, V = X⁽ˡ⁾)
```
...
```text
Δr_H = MHA(Q = r_H, K = L, V = L)
```


```text
Δr_L = MHA(Q = r_L, K = H, V = H)
```
...
Heavy summary `r_H` が Light 残基 `L₁, …, Lₘ` を読む場合、

```text
sⱼ = ((r_H W_Q)(Lⱼ W_K)ᵀ) / √dₕ
```


```text
aⱼ = exp(sⱼ) / Σₖ₌₁ᵐ exp(sₖ)
```
...
```text
x_aux → Linear(p,64) → GELU → Dropout(0.2) → Linear(64,32) → GELU
```
...
```text
H(s) = Σ_{i: ‖s−xᵢ‖ ≤ 7 Å} πᵢ · exp(−‖s−xᵢ‖)
```

を計算する。

`πᵢ` は residue-level Fauchère–Pliska hydrophobicity を
...
```text
Scratch → AbLang2 → ESM-2 → structure-aware PLM
```
...
```text
αᵢⱼ = softmaxⱼ((qᵢᵀ kⱼ) / √d)
```


```text
x′ᵢ = xᵢ + Σⱼ αᵢⱼ W_V xⱼ
```
...
```text
αᵢⱼ = softmax_{j ∈ N(i)}((qᵢᵀ kⱼ) / √d + f(eᵢⱼ))
```


```text
x′ᵢ = xᵢ + Σ_{j ∈ N(i)} αᵢⱼ W_V xⱼ
```

とすればよい。

ここで edge feature `eᵢⱼ` に
...
```text
xᵢ = xᵢ(content) + xᵢ(annotation) + xᵢ(physical)
```
...
各 residue `r` の Fauchère–Pliska hydrophobicity を `πᵣ` とする。
...
```text
H(s) = Σ_{i: ‖s−xᵢ‖ ≤ 7 Å} πᵢ · exp(−α ‖s−xᵢ‖)
```
...
```text
qᵢ(pH) = −1 / (1 + 10^(pKₐ,ᵢ − pH))
```

塩基性 residue:

```text
qᵢ(pH) = +1 / (1 + 10^(pH − pKₐ,ᵢ))
```
...
Heavy CDR3 の ESM-2 residue embedding `eᵢ` を
RASA `Rᵢ` で重み付け。
