#!/usr/bin/env python3
"""EXP-T071: Joint H/L + chain-specific REG attention mask (vs REPLAY / T070).

Same layout as T070 but REG_H/REG_L may only query their own chain.
No fusion/RASA/3D. Does NOT overwrite historical EXP-T030.
"""
...
T071_EID = "TRF_TM_ABLINGUA_FULL_JOINT_CHAIN_SPECIFIC_DUAL_REG"
VARIANT = "T071_JOINT_DUAL"  # historical run id string; architecture is chain-specific (see config flags)
INPUT_SPACE = "JOINT_HL_CHAIN_SPECIFIC_DUAL_REG_FROZEN_RESIDUE"

OUT_RUN = ROOT / "results" / "EXP-T071_run"
FREEZE_PATH = ROOT / "results" / "EXP-T071_CV_FREEZE.yaml"
REPORT_PATH = ROOT / "results" / "EXP-T071_CHAIN_SPECIFIC_REG_REPORT.md"
STATE_PATH = ROOT / "results" / "EXP-T071_run_state.json"
CKPT_MANIFEST = ROOT / "results" / "EXP-T071_CHECKPOINT_MANIFEST.json"
...
def param_counts() -> dict[str, int]:
    """Trainable params: T030 / T068 / T070 / T071."""
    kw = dict(
        content_mode="frozen",
        plm_hidden=1280,
...
def issue_t071() -> str:
    codes = load_codes()
    if (codes["experiment_id"] == T071_EID).any():
        return str(codes.set_index("experiment_id").loc[T071_EID, "experiment_code"])
    nxt = next_code(TARGET)
    if nxt != "EXP-T071":
        raise SystemExit(f"expected EXP-T071, got {nxt}")
    code = issue_code(
        T071_EID,
        TARGET,
        source_model_id=f"JOINT_HL_CHAIN_SPECIFIC_DUAL_REG::{REPLAY_ID}",
        phase="ARCHITECTURE_JOINT_HL_CHAIN_SPECIFIC_DUAL_REG",
        notes=(
            "Joint H/L chain-specific dual-REG vs contemporary EXP-T030-REPLAY-001; "
...
def write_config(code: str) -> Path:
    cfg = {
        ...
        "representation_note": (
            ...
            "results/EXP-T071_run/checkpoints/ if size permits"
        ),
...
def fold_deltas(dev, fmap, oof_new, oof_ctl, *, new_key="mae_t071", ctl_key="mae_control"):
...
def phase_freeze(code: str) -> dict:
    ...
    params = t071.get("param_counts") or param_counts()
    d_p = t071["cv_primary_mae"] - replay["cv_primary_mae"]
    d_s = t071["cv_shadow_mae"] - replay["cv_shadow_mae"]
    d_w = t071["cv_worst_mae"] - replay["cv_worst_mae"]
    ...
    freeze = {
        ...
        "param_counts": params,
        "EXP-T071": {
            "cv_primary_mae": t071["cv_primary_mae"],
            "cv_shadow_mae": t071["cv_shadow_mae"],
            "cv_mean_mae": t071["cv_mean_mae"],
            "cv_worst_mae": t071["cv_worst_mae"],
            "config_hash": t071["config_hash"],
            "best_epochs_primary": t071["best_epochs_primary"],
            "n_trainable_parameters": t071.get("n_trainable_parameters"),
        },
        "deltas_vs_EXP-T030-REPLAY-001": {"primary": d_p, "shadow": d_s, "worst": d_w},
        "deltas_vs_historical_EXP-T030": {
            "primary": t071["cv_primary_mae"] - hist["cv_primary_mae"],
            "shadow": t071["cv_shadow_mae"] - hist["cv_shadow_mae"],
            "worst": t071["cv_worst_mae"] - hist["cv_worst_mae"],
        },
        "deltas_vs_EXP-T070": {
            "primary": t071["cv_primary_mae"] - scores_row(T070_CODE)["cv_primary_mae"],
            "shadow": t071["cv_shadow_mae"] - scores_row(T070_CODE)["cv_shadow_mae"],
            "worst": t071["cv_worst_mae"] - scores_row(T070_CODE)["cv_worst_mae"],
        },
        "deltas_vs_EXP-T068": {
            "primary": t071["cv_primary_mae"] - t068["cv_primary_mae"],
            "shadow": t071["cv_shadow_mae"] - t068["cv_shadow_mae"],
            "worst": t071["cv_worst_mae"] - t068["cv_worst_mae"],
        },
        "bootstrap": bootstrap,
        "cv_scientific_verdict": verdict,
        "selection_statement": (
            "CV completed for EXP-T071; config frozen; Public/Private not used before freeze; "
...
def phase_test(code: str, *, quick: bool) -> dict:
    ...
    if pred_path.exists() and not quick:
        ...
        state = load_state()
        best = state["t071_cv"]["best_epochs_primary"]
        dev, test, folds, rb = prepare_bundle()
    ...
    n_params = freeze.get("EXP-T071", {}).get("n_trainable_parameters")
    write_checkpoint_manifest(code, ckpt_dir, n_params)
    FREEZE_PATH.write_text(yaml.safe_dump(freeze, sort_keys=False), encoding="utf-8")
    state = load_state()
    state["t071_test"] = scores
    save_state(state)
...
def phase_artifacts(code: str) -> dict:
    ...
    t070_p = pd.read_csv(ROOT / "experiments" / "predictions" / T070_CODE / "oof_primary.csv")
    t070_s = pd.read_csv(ROOT / "experiments" / "predictions" / T070_CODE / "oof_shadow.csv")
    for d in (t070_p, t070_s):
        d["id"] = d["id"].astype(str)
    col70 = TARGET if TARGET in t070_p.columns else [c for c in t070_p.columns if c != "id"][0]
    t070_ps = t070_p.set_index("id").loc[ids, col70]
    t070_ss = t070_s.set_index("id").loc[ids, col70]
    ...
    diag = {
        ...
        "fold_mae_deltas_vs_t070": {
            "primary": fold_deltas(dev, folds.primary, new_ps, t070_ps, new_key="mae_t071", ctl_key="mae_t070"),
            "shadow": fold_deltas(dev, folds.shadow, new_ss, t070_ss, new_key="mae_t071", ctl_key="mae_t070"),
        },
        "fold_mae_deltas_vs_t068": {
            "primary": fold_deltas(dev, folds.primary, new_ps, t068_ps, new_key="mae_t071", ctl_key="mae_t068"),
            "shadow": fold_deltas(dev, folds.shadow, new_ss, t068_ss, new_key="mae_t071", ctl_key="mae_t068"),
        },
    }
    ...
    assert abs(recomputed_p - freeze["EXP-T071"]["cv_primary_mae"]) < 1e-7
    assert abs(recomputed_s - freeze["EXP-T071"]["cv_shadow_mae"]) < 1e-7
    ...
    cv_p = float(freeze["EXP-T071"]["cv_primary_mae"])
    cv_s = float(freeze["EXP-T071"]["cv_shadow_mae"])
    cv_m = float(freeze["EXP-T071"]["cv_mean_mae"])
    cv_w = float(freeze["EXP-T071"]["cv_worst_mae"])
    params = freeze.get("param_counts") or param_counts()

    ckpt_dir = OUT_RUN / "checkpoints"
    ckpts = sorted(ckpt_dir.glob("fulldev_seed*.pt")) if ckpt_dir.exists() else []
    write_checkpoint_manifest(code, ckpt_dir, freeze["EXP-T071"].get("n_trainable_parameters"))
    rep_note = (
        f"representation_status=NOT_EXPORTED; checkpoints under "
        f"results/EXP-T071_run/checkpoints/ n={len(ckpts)}"
        if ckpts
    ...
    row.update(
        {
            ...
            "target": TARGET,
            "family": "TRANSFORMER",
            "model_type": "AnnotatedTransformer+JointHLChainSpecificDualREG",
            "source_model_id": f"JOINT_HL_CHAIN_SPECIFIC_DUAL_REG::{REPLAY_ID}",
            "feature_set_id": "",
            "source_recipe_id": "",
            "cv_primary_mae": cv_p,
            ...
            "score_source": "EXP-T071_cv+solution_postfreeze",
            "prediction_source": "EXP-T071_run",
            "feature_source": "ablingua_residue_joint_hl_chain_specific_dual_reg",
            "license_status": "REVIEW",
            "license_reference": "AbLingua residue",
            "notes": (
                f"Joint H/L chain-specific dual-REG vs {REPLAY_ID}; secondary vs EXP-T068; secondary vs historical {HIST_CONTROL}; "
                f"params T030={params['EXP-T030_n_trainable']} T068={params['EXP-T068_n_trainable']} T071={params['EXP-T071_n_trainable']}; "
                f"{rep_note}"
            ),
            "transformer_type": "FROZEN_PLM",
            "plm_source": "ABLINGUA",
        ...
        crow.update(
            {
                ...
                "shareability_status": "SHAREABLE_COMPLETE",
                "canonical_benchmark_eligible": "YES",
                "notes": "JOINT_HL_CHAIN_SPECIFIC_DUAL_REG: no fusion feature parquet",
            }
        )
...
def phase_report(code: str) -> None:
    ...
    lines = [
        ...
        f"- secondary: `{T070_CODE}`, `{T068_CODE}`, historical `{HIST_CONTROL}`",
        ...
        "",
        f"- T071 P/S/mean/W: {t071['cv_primary_mae']:.6f} / {t071['cv_shadow_mae']:.6f} / "
        f"{t071['cv_mean_mae']:.6f} / {t071['cv_worst_mae']:.6f}",
        ...
        "",
        f"- Δ Primary/Shadow/worst: {t071['cv_primary_mae'] - hist['cv_primary_mae']:+.6f} / "
        f"{t071['cv_shadow_mae'] - hist['cv_shadow_mae']:+.6f} / "
        f"{t071['cv_worst_mae'] - hist['cv_worst_mae']:+.6f}",
        "",
        "## Paired bootstrap",
        "",
        ...
        f"- vs T070 Primary: Δ={diag['bootstrap_vs_t070']['primary']['mae_delta']:+.6f} "
        f"CI95=[{diag['bootstrap_vs_t070']['primary']['ci95_low']:+.6f}, "
        f"{diag['bootstrap_vs_t070']['primary']['ci95_high']:+.6f}]",
        f"- vs T070 Shadow: Δ={diag['bootstrap_vs_t070']['shadow']['mae_delta']:+.6f} "
        f"CI95=[{diag['bootstrap_vs_t070']['shadow']['ci95_low']:+.6f}, "
        f"{diag['bootstrap_vs_t070']['shadow']['ci95_high']:+.6f}]",
        ...
        "## Interpretation",
        "",
        f"CV vs REPLAY **{verdict}**. Chain-specific REG mask vs unrestricted dual REG (T070).",
        "",
...
def main() -> int:
    ...
    if args.phase == "issue":
        ...
        return 0
    if args.phase == "cv":
        code = load_state().get("experiment_code") or issue_t071()
    ...
    phase_report(code)
    print("DONE EXP-T071", flush=True)
    return 0
