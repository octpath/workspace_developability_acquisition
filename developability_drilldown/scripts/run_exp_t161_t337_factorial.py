#!/usr/bin/env python3
"""Unattended factorial runner EXP-T161–T337 (Representation × Topology × Annotation).

Phases:
  prereg | gate | train | analyze | external | all

Idempotent: completed cells skipped; REUSE never overwritten.
"""
from __future__ import annotations

import argparse
import csv
import hashlib
import json
import os
import subprocess
import sys
from pathlib import Path

os.environ.setdefault("CUDA_VISIBLE_DEVICES", "0")

import numpy as np
import pandas as pd
import yaml

ROOT = Path(__file__).resolve().parents[1]
REPO = ROOT.parent
sys.path.insert(0, str(ROOT / "scripts"))
sys.path.insert(0, str(ROOT / "models"))
sys.path.insert(0, str(ROOT))

from _lib import EXPERIMENTS_COLUMNS, mae  # noqa: E402
from experiment_codes import issue_code, load_codes, next_code  # noqa: E402
from antibody_transformer.config import BUNDLE_ROOT, RESIDUE_ROOT  # noqa: E402
from antibody_transformer.data import (  # noqa: E402
    load_dev_test,
    load_folds,
    load_residue_bundle,
    load_solution,
)
from antibody_transformer.protocol_v3 import DEFAULT_SEED, PLATFORM_ID, run_protocol_v3  # noqa: E402
from preregister_t161_t337_factorial import TOPOLOGY_FLAGS, build_cells  # noqa: E402

PLAN_CSV = ROOT / "results/TMAPP_REP_TOPO_ANNOT_FACTORIAL_PLAN.csv"
PLAN_YAML = ROOT / "results/TMAPP_REP_TOPO_ANNOT_FACTORIAL_PLAN.yaml"
SEED = DEFAULT_SEED
GATE_CODES = ("EXP-T210", "EXP-T321")

NEED_KW = {
    "ablingua": "need_ablingua",
    "ablang2": "need_ablang2",
    "ablang2_unpaired": "need_ablang2_unpaired",
    "ablang1": "need_ablang1",
    "esm1b": "need_esm1b",
    "esm2": "need_esm2",
    "esmc600m": "need_esmc600m",
    "currab": "need_currab",
    "currab_unpaired": "need_currab_unpaired",
}


def _norm_plm_source(row: dict) -> str | None:
    ps = row.get("plm_source")
    if row.get("content_mode") == "scratch":
        return None
    if ps is None or (isinstance(ps, float) and pd.isna(ps)):
        return None
    s = str(ps).strip()
    if s in ("", "nan", "None", "NONE"):
        return None
    return s


def _plm_source_label(row: dict) -> str:
    ps = _norm_plm_source(row)
    return "NONE" if ps is None else ps.upper()


def git_rev() -> str:
    try:
        return subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=REPO, text=True).strip()
    except Exception:
        return "UNKNOWN"


def device_str() -> str:
    import torch

    return "cuda" if torch.cuda.is_available() else "cpu"


def load_plan() -> pd.DataFrame:
    if not PLAN_CSV.exists():
        raise SystemExit("missing plan CSV — run prereg first")
    return pd.read_csv(PLAN_CSV)


def save_plan(df: pd.DataFrame) -> None:
    tmp = PLAN_CSV.with_suffix(".csv.tmp")
    df.to_csv(tmp, index=False)
    tmp.replace(PLAN_CSV)


def save_pred(ids, vals, path: Path, col: str = "TmApp") -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    pd.DataFrame({"id": ids, col: vals}).to_csv(path, index=False)


def score_external(pred, test_ids, sol, target="TmApp"):
    sol2 = sol.set_index("id")
    te = pd.Series(pred, index=test_ids)
    pub = sol2.index[sol2["is_public"].astype(bool)].tolist()
    priv = sol2.index[sol2["is_private"].astype(bool)].tolist()
    return {
        "public_mae": float(mae(sol2.loc[pub, target].to_numpy(float), te.loc[pub].to_numpy(float))),
        "private_mae": float(mae(sol2.loc[priv, target].to_numpy(float), te.loc[priv].to_numpy(float))),
        "overall_mae": float(mae(sol2.loc[test_ids, target].to_numpy(float), te.loc[test_ids].to_numpy(float))),
    }


def already_complete(code: str) -> bool:
    return (ROOT / "results" / f"{code}_OOF_EVALUATION.yaml").exists() and (
        ROOT / "experiments" / "predictions" / code / "test_primary_mean.csv"
    ).exists()


def issue_one(row: dict) -> str:
    codes = load_codes()
    eid = row["experiment_id"]
    if (codes["experiment_id"] == eid).any():
        return str(codes.set_index("experiment_id").loc[eid, "experiment_code"])
    expect = row["experiment_code"]
    # Prefer already-reserved code row if present
    if expect in set(codes["experiment_code"].astype(str)):
        return expect
    if next_code("TmApp") != expect:
        raise SystemExit(f"expected {expect}, got {next_code('TmApp')}")
    code = issue_code(
        eid,
        "TmApp",
        source_model_id="REP_TOPO_ANNOT_FACTORIAL",
        phase="T161_FACTORIAL",
        notes=row.get("notes") or f"{row['representation']}/{row['topology']}/{row['annotation']}",
    )
    if code != expect:
        raise SystemExit(code)
    return code


def write_config(row: dict) -> None:
    code = row["experiment_code"]
    flags = TOPOLOGY_FLAGS[row["topology"]]
    ps_val = _norm_plm_source(row) or ""
    ps_out = _plm_source_label(row)
    cfg = {
        "experiment_code": code,
        "experiment_id": row["experiment_id"],
        "platform_id": PLATFORM_ID,
        "target": "TmApp",
        "family": "TRANSFORMER",
        "transformer_type": "SCRATCH" if row["content_mode"] == "scratch" else "FROZEN_PLM",
        "plm_source": ps_out,
        "annotation_mode": row["annotation_mode"],
        "chain_mode": "HL",
        "merge_mode": "mean",
        "pooling_mode": "REG",
        "content_mode": row["content_mode"],
        "seed": SEED,
        "d_model": 128,
        "share_hl_encoder": True,
        "raw_plm_hidden_dim": int(row["raw_dim"]) if row["content_mode"] == "frozen" else 0,
        "representation": row["representation"],
        "representation_context": row["representation_context"],
        "topology": row["topology"],
        "no_full_dev_refit": True,
        "arch_joint_hl_single_reg": False,
        "arch_joint_hl_dual_reg": flags["joint_hl_dual_reg"],
        "arch_joint_hl_chain_specific_dual_reg": flags["joint_hl_chain_specific_dual_reg"],
        "arch_use_cross_attention_bridge": False,
        "arch_use_reg_only_cross_attention": flags["use_reg_only_cross_attention"],
        "arch_use_within_chain_extra_attention": False,
        "arch_cross_gate_mode": "learned",
        "arch_use_cross_geometry_bias": False,
        "arch_share_hl_encoder": True,
        "pair_interaction_mode": flags["pair_interaction_mode"],
        "reg_cross_variant": None,
        "plm_source_canonical": ps_val,
    }
    (ROOT / "experiments/configs" / f"{code}.yaml").write_text(
        yaml.safe_dump(cfg, sort_keys=False), encoding="utf-8"
    )


def register(code: str, row: dict, summary: dict, ext_scores: dict) -> None:
    pm = ext_scores["primary_mean"]
    oof = summary["scores"]["oof_test"]
    topo = row["topology"]
    rep = row["representation"]
    asset_map = {
        "scratch": "assets/transformer/residue_asset_manifest.yaml#annotations+scratch_sequences",
        "ablingua": "assets/transformer/residue_asset_manifest.yaml#ablingua600m",
        "ablang2_paired": "assets/transformer/residue_asset_manifest.yaml#ablang2",
        "ablang2_unpaired": "assets/transformer/residue_asset_manifest.yaml#ablang2_unpaired",
        "ablang1": "assets/transformer/residue_asset_manifest.yaml#ablang1",
        "esm1b": "assets/transformer/residue_asset_manifest.yaml#esm1b",
        "esm2": "assets/transformer/residue_asset_manifest.yaml#esm2",
        "esmc600m": "assets/transformer/residue_asset_manifest.yaml#esmc600m",
        "currab_paired": "assets/transformer/residue_asset_manifest.yaml#currab",
        "currab_unpaired": "assets/transformer/residue_asset_manifest.yaml#currab_unpaired",
    }
    topo_prefix = {
        "A": "SEPARATE_DUAL_REG",
        "B1": "JOINT_HL_DUAL_REG",
        "B2": "JOINT_HL_CHAIN_SPECIFIC_DUAL_REG",
        "C": "SEPARATE_REG_ONLY_CROSS_ATTENTION",
        "D": "PAIR_D3",
    }[topo]
    if rep == "scratch":
        input_space = "PAIR_D3_SCRATCH_MEAN_V3" if topo == "D" else f"{topo_prefix}_SCRATCH_RESIDUE_MEAN_V3"
    elif topo == "D":
        input_space = f"PAIR_D3_FROZEN_{rep.upper()}_MEAN_V3"
    else:
        input_space = f"{topo_prefix}_FROZEN_{rep.upper()}_RESIDUE_MEAN_V3"
    exp_path = ROOT / "results/experiments.csv"
    df = pd.read_csv(exp_path)
    df = df[df["experiment_code"] != code]
    rec = {c: "" for c in df.columns}
    for c in EXPERIMENTS_COLUMNS:
        rec.setdefault(c, "")
    rec.update(
        {
            "experiment_code": code,
            "experiment_id": row["experiment_id"],
            "target": "TmApp",
            "family": "TRANSFORMER",
            "model_type": "TRANSFORMER",
            "config_path": f"experiments/configs/{code}.yaml",
            "artifact_status": "FULL",
            "cv_primary_mae": oof["primary"],
            "cv_shadow_mae": oof["shadow"],
            "cv_mean_mae": oof["mean"],
            "cv_worst_mae": oof["worst"],
            "public_mae": pm["public_mae"],
            "private_mae": pm["private_mae"],
            "test_overall_mae": pm["overall_mae"],
            "public_private_delta": pm["public_mae"] - pm["private_mae"],
            "public_private_gap": abs(pm["public_mae"] - pm["private_mae"]),
            "cv_protocol": "dl_foldlocal_cosine_v3_oof_test",
            "selection_policy_at_creation": "CV_SELECTED_POSTCOMP_EVALUATED",
            "current_evaluation_mode": "POSTCOMP_EXPLORATORY",
            "license_status": "OK",
            "source_reproducible": "YES",
            "drilldown_reproducible": "YES",
            "reproduction_status": "REPRODUCED",
            "oof_primary_path": f"experiments/predictions/{code}/oof_primary.csv",
            "oof_shadow_path": f"experiments/predictions/{code}/oof_shadow.csv",
            "test_prediction_path": f"experiments/predictions/{code}/test.csv",
            "shareability_status": "SHAREABLE_COMPLETE",
            "canonical_benchmark_eligible": "YES",
            "notes": f"factorial {row['representation']}/{row['topology']}/{row['annotation']}",
            "transformer_type": "SCRATCH" if row["content_mode"] == "scratch" else "FROZEN_PLM",
            "plm_source": _plm_source_label(row),
            "representation_status": "NOT_EXPORTED",
            "prediction_reproduction_max_delta": 0.0,
            "input_space": input_space,
            "input_asset_ref": asset_map[rep],
        }
    )
    pd.concat([df, pd.DataFrame([{c: rec.get(c, "") for c in df.columns}])], ignore_index=True).to_csv(
        exp_path, index=False
    )
    comp_path = ROOT / "results/EXPERIMENT_ARTIFACT_COMPLETENESS.csv"
    if comp_path.exists():
        comp = pd.read_csv(comp_path)
        if code not in set(comp["experiment_code"].astype(str)):
            crow = {c: "" for c in comp.columns}
            crow.update(
                {
                    "experiment_code": code,
                    "experiment_id": row["experiment_id"],
                    "target": "TmApp",
                    "family": "TRANSFORMER",
                    "config_exists": True,
                    "feature_required": False,
                    "feature_exists": True,
                    "oof_primary_exists": True,
                    "oof_shadow_exists": True,
                    "test_exists": True,
                    "score_recompute_ok": True,
                    "reproduction_status": "REPRODUCED",
                    "shareability_status": "SHAREABLE_COMPLETE",
                    "canonical_benchmark_eligible": "YES",
                    "notes": "rep×topo×annot factorial",
                }
            )
            if "test_prediction_exists" in comp.columns:
                crow["test_prediction_exists"] = True
            comp = pd.concat([comp, pd.DataFrame([{c: crow.get(c, "") for c in comp.columns}])], ignore_index=True)
            comp.to_csv(comp_path, index=False)


def run_cell(row: dict, *, rb, folds, dev, test, quick: bool = False) -> dict:
    code = row["experiment_code"]
    eid = (
        f"TRF_TM_{row['representation'].upper()}_{row['annotation']}_{row['topology']}_MEAN_V3"
    )
    row = dict(row)
    row["experiment_id"] = eid
    write_config(row)
    if already_complete(code) and not quick:
        oof = yaml.safe_load((ROOT / f"results/{code}_OOF_EVALUATION.yaml").read_text())
        ot = oof.get("oof_test") or oof["scores"]["oof_test"]
        return {"code": code, "status": "COMPLETE", "oof_test": ot, "skipped": True}

    code = issue_one(row)
    print(f"==== TRAIN {code} {row['representation']}/{row['topology']}/{row['annotation']} ====", flush=True)
    out = ROOT / "results" / f"{code}_run"
    try:
        result = run_protocol_v3(
            experiment_code=code,
            target="TmApp",
            dev=dev,
            test=test,
            folds=folds,
            rb=rb,
            device=device_str(),
            out_dir=out,
            seed=SEED,
            quick=quick,
            arch=TOPOLOGY_FLAGS[row["topology"]],
            content_mode=row["content_mode"],
            merge_mode="mean",
            plm_source=_norm_plm_source(row),
            annotation_mode=row["annotation_mode"],
            chain_mode="HL",
        )
    except Exception as e:
        print(f"FAILED {code}: {e}", flush=True)
        return {"code": code, "status": "FAILED", "error": str(e)}

    summary = result["summary"]
    sel = result["selected_df"]
    sel.to_csv(ROOT / f"results/{code}_SELECTED_LR.csv", index=False)
    pred = ROOT / "experiments/predictions" / code
    pred.mkdir(parents=True, exist_ok=True)
    save_pred(result["dev_ids"], result["oof_test"]["primary"].loc[result["dev_ids"]].to_numpy(float), pred / "oof_primary.csv")
    save_pred(result["dev_ids"], result["oof_test"]["shadow"].loc[result["dev_ids"]].to_numpy(float), pred / "oof_shadow.csv")
    for scheme in ("primary", "shadow"):
        save_pred(result["dev_ids"], result["oof_val"][scheme].loc[result["dev_ids"]].to_numpy(float), pred / f"oof_val_{scheme}.csv")
        save_pred(result["dev_ids"], result["oof_test"][scheme].loc[result["dev_ids"]].to_numpy(float), pred / f"oof_test_{scheme}.csv")
    for key in ("primary_mean", "primary_median", "shadow_mean", "shadow_median"):
        save_pred(result["test_ids"], result["ext"][key], pred / f"test_{key}.csv")
    save_pred(result["test_ids"], result["ext"]["primary_mean"], pred / "test.csv")

    # sanity: nonzero variance
    pvar = float(np.nanvar(result["oof_test"]["primary"].loc[result["dev_ids"]].to_numpy(float)))
    if not np.isfinite(pvar) or pvar <= 0:
        return {"code": code, "status": "FAILED", "error": f"nonpositive pred variance {pvar}"}

    sol = load_solution(BUNDLE_ROOT / "solution.csv")
    ext_scores = {
        key: score_external(result["ext"][key], result["test_ids"], sol)
        for key in ("primary_mean", "primary_median", "shadow_mean", "shadow_median")
    }
    ot = summary["scores"]["oof_test"]
    oof_doc = {
        "experiment_code": code,
        "representation": row["representation"],
        "topology": row["topology"],
        "annotation": row["annotation"],
        "oof_test": ot,
        "oof_val": summary["scores"]["oof_val"],
        "external": ext_scores,
        "n_trainable": summary.get("n_trainable"),
        "param_account": summary.get("param_account"),
        "selected": sel.to_dict(orient="records"),
        "prediction_variance_oof_primary": pvar,
    }
    (ROOT / f"results/{code}_OOF_EVALUATION.yaml").write_text(yaml.safe_dump(oof_doc, sort_keys=False), encoding="utf-8")
    (ROOT / f"results/{code}_run_state.json").write_text(json.dumps({"ext_scores": ext_scores}, indent=2))
    register(code, row, summary, ext_scores)
    print(f"DONE {code} mean={ot['mean']:.6f}", flush=True)
    return {"code": code, "status": "COMPLETE", "oof_test": ot, "skipped": False}


def load_bundle_for_rows(rows: list[dict], dev, test):
    need = {}
    for r in rows:
        ps = _norm_plm_source(r) or ""
        if not ps or r["content_mode"] == "scratch":
            continue
        kw = NEED_KW.get(ps)
        if kw:
            need[kw] = True
    return load_residue_bundle(dev, test, **need)


def update_plan_row(df: pd.DataFrame, code: str, status: str, oof=None) -> pd.DataFrame:
    idx = df.index[df["experiment_code"] == code]
    if len(idx) != 1:
        return df
    i = idx[0]
    df.at[i, "execution_status"] = status
    if oof:
        df.at[i, "primary_mae"] = oof["primary"]
        df.at[i, "shadow_mae"] = oof["shadow"]
        df.at[i, "mean_ps"] = oof["mean"]
        df.at[i, "worst_ps"] = oof["worst"]
    save_plan(df)
    return df


def fill_reuse_scores(df: pd.DataFrame) -> pd.DataFrame:
    exp = pd.read_csv(ROOT / "results/experiments.csv")
    for i, row in df.iterrows():
        if row["execution_status"] != "REUSE":
            continue
        code = row["reuse_source"] or row["experiment_code"]
        r = exp[exp.experiment_code == code]
        if len(r) != 1:
            continue
        r = r.iloc[0]
        df.at[i, "primary_mae"] = r.cv_primary_mae
        df.at[i, "shadow_mae"] = r.cv_shadow_mae
        df.at[i, "mean_ps"] = r.cv_mean_mae
        df.at[i, "worst_ps"] = r.cv_worst_mae
        df.at[i, "execution_status"] = "REUSE"
    save_plan(df)
    return df


def phase_prereg() -> None:
    subprocess.check_call(
        [sys.executable, str(ROOT / "scripts/preregister_t161_t337_factorial.py")],
        cwd=str(ROOT),
    )
    # write configs for planned
    df = load_plan()
    for _, row in df[df.execution_status == "PLANNED"].iterrows():
        r = row.to_dict()
        r["experiment_id"] = f"TRF_TM_{r['representation'].upper()}_{r['annotation']}_{r['topology']}_MEAN_V3"
        write_config(r)
    fill_reuse_scores(df)
    print("prereg complete", flush=True)


def reserve_all_planned_ids(df: pd.DataFrame) -> None:
    """Issue EXP-T161..T337 sequentially so gate cells T210/T321 can run first."""
    planned = df[df.execution_status == "PLANNED"].sort_values("experiment_code")
    codes = load_codes()
    issued = set(codes["experiment_code"].astype(str))
    for _, row in planned.iterrows():
        code = row["experiment_code"]
        if code in issued:
            continue
        eid = f"TRF_TM_{row['representation'].upper()}_{row['annotation']}_{row['topology']}_MEAN_V3"
        if (codes["experiment_id"] == eid).any():
            continue
        expect = next_code("TmApp")
        if expect != code:
            raise SystemExit(f"reserve mismatch: want {code}, next={expect}")
        got = issue_code(
            eid,
            "TmApp",
            source_model_id="REP_TOPO_ANNOT_FACTORIAL",
            phase="T161_FACTORIAL_RESERVE",
            notes=f"reserve {row['representation']}/{row['topology']}/{row['annotation']}",
        )
        if got != code:
            raise SystemExit(got)
        issued.add(code)
        codes = load_codes()
    print(f"reserved planned IDs; next={next_code('TmApp')}", flush=True)


def phase_gate(*, quick: bool = False) -> None:
    df = load_plan()
    reserve_all_planned_ids(df)
    subprocess.check_call(["bash", str(RESIDUE_ROOT / "assemble_embeddings.sh")])
    # ensure matched assets exist
    for sub in ("ablang2_unpaired", "currab_unpaired"):
        if not (RESIDUE_ROOT / sub / "metadata.json").exists():
            raise SystemExit(f"missing {sub} — run extract_matched_unpaired_residue_embeddings.py")
    dev, test = load_dev_test(ROOT / "data/dev.csv", ROOT / "data/test.csv")
    folds = load_folds(ROOT / "data/folds.csv")
    gate_rows = [df[df.experiment_code == c].iloc[0].to_dict() for c in GATE_CODES]
    rb = load_bundle_for_rows(gate_rows, dev, test)
    for row in gate_rows:
        res = run_cell(row, rb=rb, folds=folds, dev=dev, test=test, quick=quick)
        df = update_plan_row(df, row["experiment_code"], res["status"], res.get("oof_test"))
        if res["status"] != "COMPLETE":
            raise SystemExit(f"GATE FAIL {row['experiment_code']}: {res}")
    print("GATE PASS", flush=True)


def phase_train(*, quick: bool = False, only_codes: list[str] | None = None) -> None:
    df = load_plan()
    # gate must be complete
    for c in GATE_CODES:
        st = df.loc[df.experiment_code == c, "execution_status"].iloc[0]
        if st not in ("COMPLETE", "REUSE"):
            # COMPLETE after gate; allow resume if already done
            if not already_complete(c):
                raise SystemExit(f"gate cell {c} status={st}; run gate first")
    subprocess.check_call(["bash", str(RESIDUE_ROOT / "assemble_embeddings.sh")])
    dev, test = load_dev_test(ROOT / "data/dev.csv", ROOT / "data/test.csv")
    folds = load_folds(ROOT / "data/folds.csv")
    planned = df[df.execution_status.isin(["PLANNED", "FAILED"])].copy()
    if only_codes:
        planned = planned[planned.experiment_code.isin(only_codes)]
    # also re-include gate if somehow incomplete
    rows = planned.to_dict(orient="records")
    # group by plm needs to avoid reloading constantly — load all needed once
    rb = load_bundle_for_rows(df.to_dict(orient="records"), dev, test)
    for row in rows:
        if already_complete(row["experiment_code"]) and row["execution_status"] != "FAILED":
            oof = yaml.safe_load((ROOT / f"results/{row['experiment_code']}_OOF_EVALUATION.yaml").read_text())
            ot = oof.get("oof_test") or oof["scores"]["oof_test"]
            df = update_plan_row(df, row["experiment_code"], "COMPLETE", ot)
            continue
        res = run_cell(row, rb=rb, folds=folds, dev=dev, test=test, quick=quick)
        # one safe retry on transient failure
        if res["status"] == "FAILED":
            print(f"retry once {row['experiment_code']}", flush=True)
            res = run_cell(row, rb=rb, folds=folds, dev=dev, test=test, quick=quick)
        df = update_plan_row(df, row["experiment_code"], res["status"], res.get("oof_test"))
    print("train phase done", flush=True)


def main() -> int:
    ap = argparse.ArgumentParser()
    ap.add_argument("phase", choices=["prereg", "gate", "train", "analyze", "external", "all"])
    ap.add_argument("--quick", action="store_true")
    args = ap.parse_args()
    if args.phase in ("prereg", "all"):
        phase_prereg()
    if args.phase in ("gate", "all"):
        phase_gate(quick=args.quick)
    if args.phase in ("train", "all"):
        phase_train(quick=args.quick)
    if args.phase in ("analyze", "all"):
        from analyze_t161_t337_factorial import run_analysis

        run_analysis()
    if args.phase == "external":
        from analyze_t161_t337_factorial import run_external

        run_external()
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
