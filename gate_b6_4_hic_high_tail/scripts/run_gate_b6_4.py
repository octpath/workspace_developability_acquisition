#!/usr/bin/env python3
"""Gate B6.4 — HIC high-tail mechanism & learnability audit."""
...
def build_pop(pack):
    ...
    pop = pop.rename(columns={"heavy": "VH", "light": "VL"})
    role = {}
    ...
    print("pred tags:", sorted(preds.keys()))
    for tag in ALLM:
        if tag not in preds:
            raise KeyError(f"missing pred tag {tag}; available={sorted(preds.keys())}")
        by = {}
        for rk, ids in [("cv", meta["train_ids"]), ("public", meta["public_ids"]), ("private", meta["private_ids"])]:
            if rk not in preds[tag]:
                raise KeyError(f"missing role {rk} for {tag}; available={list(preds[tag].keys())}")
            for i, v in zip(ids, preds[tag][rk]):
                by[i] = float(v)
    ...
    print("psr columns:", list(psr.columns)[:12])
    ...
    cdrs = [c for c in ["H_CDR1", "H_CDR2", "H_CDR3", "L_CDR1", "L_CDR2", "L_CDR3"] if c in num.columns]
    if not cdrs:
        print("WARNING numbering CDR missing; available=", list(num.columns)[:40])
    pop = pop.merge(num[["id"] + cdrs], on="id", how="left")
    ...
    miss = [c for c in want if c not in sasa.columns]
    if miss:
        print("WARNING missing SASA", miss)
        print("available:", [c for c in sasa.columns if "hydrophobic" in c or "patch" in c][:40])
...
def phase1(pop):
    ...
    for need in ["Clone name", "HIC retention time (min)", "VH Protein", "VL Protein"]:
        if need not in idx:
            raise KeyError(f"mmc2 missing {need}; header={hdr}")
    ...
    for _, r in hi.iterrows():
        ...
        def norm_aa(x):
            # strip spaces and gap characters that appear in mmc2 aligned VL strings
            return "".join(ch for ch in str(x).upper() if ch.isalpha())
        vhok = norm_aa(s[idx["VH Protein"]]) == norm_aa(r.VH)
        vlok = norm_aa(s[idx["VL Protein"]]) == norm_aa(r.VL)
    ...
    (REPORTS / "01_high_hic_integrity.md").write_text(
        f"# 01 Integrity\n\nn_HIGH={len(hi)}; exact HIC matches={int(ver_df.exact_match.sum())}/13; "
        f"sequence matches={int(ver_df.sequence_match.sum())}/13; issues={int((ver_df.any_issue!='NONE').sum())}.\n\n"
        f"Order stats:\n\n{os_df.to_markdown(index=False)}\n\n"
        "Interpretation: HIGH forms a smooth right-tail continuation above 11.5, not an isolated gap artifact.\n"
    )
...
def effects(pop):
    feats = [c for c in pop.columns if any(c.startswith(p) for p in
             ("VH_", "VL_", "HL_", "H_CDR3_", "L_CDR3_", "PL_", "ESMFN_"))
             and pd.api.types.is_numeric_dtype(pop[c])]
    ...
    seq_mask = edf.feature.str.startswith(("VH_", "VL_", "HL_", "H_CDR3_", "L_CDR3_", "PL_"))
    edf[seq_mask].to_csv(METRICS / "high_hic_sequence_features.csv", index=False)
    for f in edf.head(8).feature:
        ...
        fig.savefig(PLOTS / "high_hic_feature_effects" / f"{str(f)[:70].replace('/', '_')}.png", dpi=120); plt.close(fig)
    (REPORTS / "02_high_hic_sequence_mechanism.md").write_text(
        "# 02 Sequence mechanism\n\n" + edf[seq_mask].head(25).to_markdown(index=False) + "\n"
    )
    return edf
...
def shm(pop):
    ...
    note = ("Per-mutation hydrophobicity-increasing SHM classification was NOT run: frozen germline AA "
            "alignments unavailable for all 324. Germline_distance/mutfrac used as proxies.")
...
def disagreement(pop):
    ...
    (REPORTS / "06_high_hic_model_failure_analysis.md").write_text("# 06 Model failures\n\n" + ddf.to_markdown(index=False) + "\n")
    return ddf
...
def neighbors(pop, train_ids):
    ...
    if len(ddf):
        ddf = ddf.sort_values("abs_HIC_diff", ascending=False)
    else:
        ddf = pd.DataFrame(columns=["id_a", "id_b", "paired_sim", "HIC_a", "HIC_b", "abs_HIC_diff"])
...
def diagnostics(pop, train_ids):
    ...
    print("PHYSSEQ:", physseq)
    print("PHYSSTRUCT:", physstruct)
    ...
    for name, X, kind, elevw in specs:
        ...
        for tr, te in gkf.split(X, y, groups):
            pipe = Pipeline([("sc", StandardScaler()), ("m", {
                "ridge": Ridge(10.0), "enet": ElasticNet(alpha=0.05, l1_ratio=0.5, max_iter=20000),
        ...
        results.append(evaluate(name, oof))
    ...
    pca.fit(sc_e.fit_transform(Etr))

    Xtr = features(win, train, pca, sc_e)
    ...
    model = {"ridge": Ridge(10.0), "enet": ElasticNet(alpha=0.05, l1_ratio=0.5, max_iter=20000),
    ...
    for role, sub in [("public", pop[pop.role == "Public"]), ("private", pop[pop.role == "Private"])]:
        X = features(win, sub, pca, sc_e)
        pred = model.predict(sc.transform(X)); yy = sub.HIC.values
...
def oracle(pop, train_ids):
    ...
    subset_col = "b_cell_subset" if "b_cell_subset" in train.columns else None
    if subset_col is None:
        for c in train.columns:
            if "subset" in c.lower() or "b_cell" in c.lower():
                subset_col = c; break
    if subset_col is None:
        dummies = pd.DataFrame({"sub_UNK": np.ones(len(train))})
    else:
        dummies = pd.get_dummies(train[subset_col].fillna("UNK"), prefix="sub")
    ...
    if subset_col and subset_col in pop.columns:
        pd.crosstab(pop[subset_col], pop.true_band).to_csv(METRICS / "high_hic_bcell_subset_counts.csv")
    if "donor" in pop.columns:
...
def case_table_and_studies(pop, ndf):
    ...
    keep = ["id", "role", "HIC", "true_band", "VH", "VL", "sequence_group", "cluster_size", "vh_family", "vl_family",
            "vh_germline", "vl_germline", "vh_len", "vl_len", "TmApp", "PSR",
            "consensus_severe_failure", "n_adv_pred_LOW", "n_adv_pred_MEDIUM", "n_adv_pred_HIGH"]
    for c in pop.columns:
        if c.startswith(("pred_", "resid_", "pred_band_", "HL_", "VH_", "VL_", "H_CDR3_", "PL_", "ESMFN_", "b_cell")):
            keep.append(c)
    cols, seen = [], set()
    for c in keep:
    ...
    for iid in ids:
        ...
        lines.append(f"- TmApp={r.TmApp}, PSR={r.PSR}\n")
        reasons = []
    ...
    for tag, col in [("NESTED_STACK_NNLS", "#4c6a8a"), ("ESMFN_STRUCTURE_ElasticNet", "#0f766e"),
                     ("PLM_ESM2_PCA64_SVR", "#7c3aed")]:
        ax.scatter(hi.HIC, hi[f"pred_{tag}"], s=36, c=col, alpha=0.75, label=tag.split("_")[0])
    ax.plot([11.5, hi.HIC.max() + 0.2], [11.5, hi.HIC.max() + 0.2], "k--", lw=1)
    ax.axhline(LOW_HI, ls=":", c="gray"); ax.axvline(HIGH_LO, ls=":", c="gray")
    ax.set_xlabel("true HIC"); ax.set_ylabel("predicted HIC"); ax.legend(fontsize=7)
    ax.set_title("HIGH cohort: pred vs true")
    fig.tight_layout(); fig.savefig(PLOTS / "high_hic_case_studies/high_pred_vs_true.png", dpi=140); plt.close(fig)
...
def final_report(pop, ver, edf, sdf, shm_df, shm_note, ndf, smooth, disc, diag, hold, freeze, oracle_row):
    ...
    win = freeze["winner"]; tw = freeze["train_metrics"]
    ...
    top_feats = ", ".join(edf.head(5).feature.tolist())
    ...
    # tolerate column-name aliases from prior runs
    exact_col = "exact_match" if "exact_match" in ver.columns else "exact_match"
    seq_col = "sequence_match" if "sequence_match" in ver.columns else "sequence_match"
    n_exact = int(ver[exact_col].sum()) if exact_col in ver.columns else int(ver.get("exact_match", pd.Series(dtype=bool)).sum())
    n_seq = int(ver[seq_col].sum()) if seq_col in ver.columns else n_exact
    h1 = "NOT SUPPORTED" if n_exact == 13 and n_seq == 13 else ("WEAK" if n_exact == 13 else "INCONCLUSIVE")
    h2 = "STRONG"
    h3 = "MODERATE" if hi_sim < low_sim - 0.01 else "WEAK"
    h4 = "MODERATE" if top_auc >= 0.70 else "WEAK"
    top_struct = float(sdf.iloc[0].cliffs_delta) if len(sdf) else 0.0
    ...
    # Evidence-based decision (labels intact; scarcity dominates; modest feature signal)
    ...
    elif float(tw["HIGH_to_LOW_rate"]) <= 0.34 and top_auc >= 0.75:
        decision = "HIC_STRONG_COMPETITION_TARGET_WITH_HEADROOM"
    ...
    n_struct_closer = int(pd.read_csv(METRICS / "high_hic_model_disagreement.csv").structure_closer.sum())
...
- structure-model advantage: ESMFN closer than PLM in {n_struct_closer}/13 HIGH cases
...
16. Structure helps some PLM misses ({n_struct_closer}/13 closer) but does not fully solve HIGH→LOW.
...
"""
    ...
    (REPORTS / "GATE_B6_4_FINAL.md").write_text(text)
...
def main():
    ...
    protocol = {
        "gate": "B6.4",
        "split": "CAND_12528",
        "bands": {"LOW": "<10.5", "MEDIUM": "10.5-11.5", "HIGH": ">11.5"},
        "constraints": [
            "read_only_prior_gates",
            "continuous_HIC_target",
            "no_classification_redesign",
            "no_new_split_search",
            "descriptive_all_N324_ok",
            "new_models_train_only_groupCV_then_oneshot_holdout",
        ],
        "advanced_models_for_consensus": ADV,
        "consensus_severe_failure": "true HIGH & >=3/5 ADVANCED predict LOW(<10.5)",
    }
    (CONFIG / "B6_4_PROTOCOL.json").write_text(json.dumps(protocol, indent=2))
    print("load")
    pack = load_b62()
    pop, meta = build_pop(pack)
    print("N", len(pop), "HIGH", int((pop.true_band == "HIGH").sum()),
          "severe", int(pop.consensus_severe_failure.sum()))
    hi, ver, _ = phase1(pop)
    matched(pop)
    edf = effects(pop)
    ...
    decision = final_report(pop, ver, edf, sdf, shm_df, shm_note, ndf, smooth, disc,
                            diag, hold, freeze, oracle_row)
    # aliases matching brief names
    alias_pairs = [
        (DATA / "high_hic_13.csv", DATA / "high_hic_13.csv"),
        (DATA / "high_hic_case_table.csv", DATA / "high_hic_case_table.csv"),
        (DATA / "high_hic_matched_controls.csv", DATA / "high_hic_matched_controls.csv"),
        (METRICS / "high_hic_source_verification.csv", METRICS / "high_hic_source_verification.csv"),
        (METRICS / "high_hic_feature_effects.csv", METRICS / "high_hic_feature_effects.csv"),
        (METRICS / "high_hic_sequence_features.csv", METRICS / "high_hic_sequence_features.csv"),
        (METRICS / "high_hic_structure_features.csv", METRICS / "high_hic_structure_features.csv"),
        (METRICS / "high_hic_matched_differences.csv", METRICS / "high_hic_matched_differences.csv"),
        (METRICS / "high_hic_shm_analysis.csv", METRICS / "high_hic_shm_analysis.csv"),
        (METRICS / "high_hic_model_disagreement.csv", METRICS / "high_hic_model_disagreement.csv"),
        (METRICS / "high_hic_neighbor_analysis.csv", METRICS / "high_hic_neighbor_analysis.csv"),
        (METRICS / "high_hic_local_smoothness.csv", METRICS / "high_hic_local_smoothness.csv"),
        (METRICS / "high_hic_diagnostic_models.csv", METRICS / "high_hic_diagnostic_models.csv"),
        (METRICS / "high_hic_diagnostic_holdout_oneshot.csv", METRICS / "high_hic_diagnostic_holdout_oneshot.csv"),
        (METRICS / "high_hic_oracle_diagnostic.csv", METRICS / "high_hic_oracle_diagnostic.csv"),
        (METRICS / "high_hic_assay_correlations.csv", METRICS / "high_hic_assay_correlations.csv"),
        (PLOTS / "high_hic_order_statistics.png", PLOTS / "high_hic_order_statistics.png"),
        (REPORTS / "01_high_hic_integrity.md", REPORTS / "01_high_hic_integrity.md"),
        (REPORTS / "GATE_B6_4_HIC_HIGH_TAIL_FINAL.md", REPORTS / "GATE_B6_4_HIC_HIGH_TAIL_FINAL.md"),
    ]
    # also map common alternate produced names if present
    alt = {
        DATA / "high_hic_13.csv": DATA / "high_hic_13.csv",
        DATA / "high_hic_case_table.csv": DATA / "high_hic_case_table.csv",
        DATA / "high_hic_matched_controls.csv": DATA / "high_hic_matched_controls.csv",
        ...
        REPORTS / "01_high_hic_integrity.md": REPORTS / "01_high_hic_integrity.md",
        REPORTS / "GATE_B6_4_HIC_HIGH_TAIL_FINAL.md": REPORTS / "GATE_B6_4_HIC_HIGH_TAIL_FINAL.md",
        REPORTS / "GATE_B6_4_FINAL.md": REPORTS / "GATE_B6_4_HIC_HIGH_TAIL_FINAL.md",
    }
    for src, dst in alt.items():
        if src.exists() and src.resolve() != dst.resolve():
            dst.write_bytes(src.read_bytes())
    print("done", REPORTS / "GATE_B6_4_HIC_HIGH_TAIL_FINAL.md", decision)
    print("KEY", json.dumps({
        "n_HIGH": 13,
        "n_verified": int(ver.exact_match.sum()) if "exact_match" in ver.columns else int(ver.exact_match.sum()),
        "severe_failures": int(pop.consensus_severe_failure.sum()),
        "winner": freeze["winner"],
        "train_HIGH_to_LOW": float(freeze["train_metrics"]["HIGH_to_LOW_rate"]),
        "decision": decision,
    }, indent=2))
