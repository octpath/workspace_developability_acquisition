#!/usr/bin/env python3
"""Shared Gate2A/2B Ridge evaluation helpers (exact nested + residual Ridge)."""
...
def nested_ridge(X, y, folds):
    ...
    model.fit(sc.fit_transform(X.loc[ids]), y.loc[ids])
...
def eval_one_target(
    ...
    frozen_oof: pd.Series,
    frozen_test: pd.Series,
    canon: Sequence[str],
    univariate_features: Sequence[str] | None = None,
):
    ...
    uni_feats = list(univariate_features) if univariate_features is not None else list(canon)
    out = {}
    audits = {}
    inc_map = {}
    for gen in GENS:
        ...
        if len(y_g) < len(y):
            print(f"  note: {gen} missing {len(y) - len(y_g)} Dev IDs vs full Dev", flush=True)
        X = feat_by_gen[gen].loc[y_g.index, list(canon)].astype(float)
        Xte = feat_by_gen[gen].loc[y_test_g.index, list(canon)].astype(float)
        oof, model, sc, a_full, alphas = nested_ridge(X, y_g, folds)
        base_oof, base_med = median_baseline_oof(y_g, folds)
        test_pred = pd.Series(model.predict(sc.transform(Xte)), index=y_test_g.index)

        m = {"alpha_full": a_full, "alphas_outer": alphas, "n_dev_used": int(len(y_g)), "n_test_used": int(len(y_test_g))}
        ...
        cand_oof, res_model, res_sc, audit = residual_oof(X, y_g, folds, frozen_oof_g, base_mat, target)
        audits[gen] = audit
        cand_test = frozen_test_g.astype(float) + pd.Series(
            res_model.predict(res_sc.transform(Xte)), index=y_test_g.index
        )
        boots = {}
        ...
        if target == "TmApp":
            ...
            low = y_g <= np.quantile(y_g, 0.2)
            high = y_g >= np.quantile(y_g, 0.8)
            m["low_Tm_bias_candidate_CV"] = float(np.mean(cand_oof[low] - y_g[low]))
            m["high_Tm_bias_candidate_CV"] = float(np.mean(cand_oof[high] - y_g[high]))
        else:
        ...
        m["prespec_univariate_Dev"] = [
            {"feature": f, "spearman_vs_y": float(spearmanr(X[f], y_g).statistic)} for f in uni_feats if f in X.columns
        ]
        out[gen] = m
...
def run_family_evaluation(family_dir: Path, family_id: str, version: str, canon: Sequence[str], univariate_features=None):
    ...
    tmapp, tmapp_audit = eval_one_target(
        "TmApp", feat_by_gen, folds, sol, tmapp_base, tmapp_oof, tmapp_test, canon, univariate_features
    )
    hic, hic_audit = eval_one_target(
        "HIC", feat_by_gen, folds, sol, hic_base, hic_oof, hic_test, canon, univariate_features
    )
    (family_dir / "EVALUATION_RESULTS.json").write_text(
        json.dumps({"family_id": family_id, "version": version, "TmApp": tmapp, "HIC": hic}, indent=2, default=str)
        + "\n"
    )
        ...
        json.dumps(
            {
                ...
                "implementation": "organizer_extension/feature_prospecting/common/ridge_eval.py",
                "reused_from": "Gate2A ANM-SPECTRUM",
            },
            indent=2,
        )
        + "\n"
